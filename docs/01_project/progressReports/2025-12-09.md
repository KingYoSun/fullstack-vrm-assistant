...
- SnapMoGen motion_service スタブと backend/frontend の初期統合を実施し、compose/.env に Motion 設定を追加。
...
- `motion_service/*`, `docker/motion-snapmogen/Dockerfile`, `docker-compose.yml`: SnapMoGen ラッパの FastAPI スタブと GPU コンテナを追加し、/data/animations へ VRM キー列 JSON を保存する初期実装を用意。
...
- SnapMoGen 本体の推論呼び出しと VRM 再生ロジック（three-vrm 連携、motion_prompt 抽出/同期）が未実装。

## 次のアクション
- SnapMoGen 推論コードと VRM 再生/同期処理を実装し、生成結果を front で実再生できるようにする。
